...
## 클래스의 상속
    ...
    - 상속 정의
      ...
      * 자바는 상속관계에 있을지라도 상위 클래스의 멤버는 
      ...
      또한 하나의 부모클래스가 여러개의 자식 클래스를 두는 것은 허용이 된다.

        * 허용이 안되는 것은 하나의 자식클래스가 여러개의 부모 클래스를 두는 것이다.
        why ? => Father클래스와 Mother클래스가 있으며 Child자식 클래스가 있다고 치자.
        Father클래스와 Mother클래스 모두 love()라는 메서드가 있을 때 Child자식 클래스는
        love()를 override할때 어떤 부모클래스로부터 love()메서드를 상속받았는지
        자바 컴파일러가 알기 힘들기 때문이다.
...
## 인터페이스
    ...
    - 인터페이스는 추상 메서드만 가지고 있는 클래스라고 생각하면 된다.

    * 추상메서드란 ?
      => interface Printable{
            public void print(String doc); 
         }
        위와 같이 몸통이 없는 메서드를 '추상 메서드'라고 일컫는다.
      
    - 인터페이스를 대상으로는 인스턴스 생성이 불가능하다.
    - 다만 , 다른 클래스에 의해 상속만이 될 뿐이다.
    
    * 인터페이스

      - 인터페이스를 상속하는 키워드는 implements이다.
      - 한 클래스는 둘 이상의 인터페이스를 구현할 수 있다.
      - 상속과 구현은 동시에 가능하다.
      - 클래스는 인터페이스와 다른 클래스 상속을 모두 할 수 있다.
      ...
      - 인터페이스의 추상 메서드와 이를 구현하는 메서드 사이에 오버라이딩 관계가 성립한다.
        => @Override annotation선언이 가능하다.
        ...
        ex)
          ...
          class Printer implements Printable{
            @Override
            public void print(String doc){
              System.out.println("interface can override!");
            }
          }

          Printable prn = new Printer(); // Printable형 참조변수 선언 가능
      
      - 인터페이스는 '연결점'으로 둘 사이를 연결하는 매개체 역할을 한다.
      - 인터페이스 간 상속도 가능하다.
      - 인터페이스의 형(type) 이름을 대상으로 instanceof 연산도 가능하다.
